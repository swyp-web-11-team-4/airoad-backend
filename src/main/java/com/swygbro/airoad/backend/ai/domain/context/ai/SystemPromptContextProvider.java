...
/**
 * 시스템 프롬프트를 제공하는 ContextProvider
 *
 * <p>AgentType에 따라 데이터베이스에서 활성화된 시스템 프롬프트를 조회하여 컨텍스트로 제공합니다.
 */
@Slf4j
@Component
public class SystemPromptContextProvider extends AbstractContextProvider<AgentType> {

  private final AiPromptTemplateQueryUseCase promptTemplateQueryUseCase;

  public SystemPromptContextProvider(AiPromptTemplateQueryUseCase promptTemplateQueryUseCase) {
  ...
  protected List<MetadataEntry> doGetContext(AgentType agentType) {
    log.debug("시스템 프롬프트 조회 - AgentType: {}", agentType);

    String systemPrompt =
        promptTemplateQueryUseCase.findActivePromptTemplate(PromptType.SYSTEM, agentType).prompt();

    log.debug("시스템 프롬프트 조회 완료 - 길이: {} 자", systemPrompt.length());

    return PromptMetadataAdvisor.systemMetadata(systemPrompt);
  }
