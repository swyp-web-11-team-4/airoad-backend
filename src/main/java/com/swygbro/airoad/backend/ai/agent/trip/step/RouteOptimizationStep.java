...
public class RouteOptimizationStep implements PipelineStep {
  ...
  public ExecutionContext execute(ExecutionContext context) {
    ...
    // 1. 전체 후보군 가져오기
    List<Document> allPlaces = new ArrayList<>(dedupResult.places());
    List<Document> allRestaurants = new ArrayList<>(dedupResult.restaurants());
    ...
    // 2. 앵커 선정
    Document anchor =
        allPlaces.stream().max(Comparator.comparingDouble(Document::getScore)).orElseThrow();
